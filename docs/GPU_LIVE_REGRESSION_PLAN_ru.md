...
Этот документ описывает план следующего live GPU validation window для текущего HEAD ветки `beta/gpu-model-validation` после Office file optimization v1.0, parser quality gate, PDF OCR v1 source implementation и Comparison Engine internal baseline / comparison quality gate.

Это не отчёт о выполненной проверке, не подтверждение результатов на GPU-стенде и не capacity benchmark. PDF OCR v1 реализован как opt-in source implementation behind feature flag, default-off; live validation pending. Comparison Engine internal baseline готов; API/UI/runtime integration pending. Цель окна - подтвердить demo / pilot validation readiness текущего regression baseline, а не провести production certification.
...
- локальный comparison quality gate после clone/pull;
- Comparison Engine internal baseline tests: normalized document model, deterministic diff и Markdown report;
...
- comparison API/UI/runtime integration remains out of scope for this window;
...
Локальные parser/comparison quality gates выполнить на validation host после clone/pull и до live smoke.
...
Comparison quality gate with parser baseline:
...
Comparison quality gate тоже является local deterministic check: он проверяет normalized document model, deterministic diff и Markdown report generator, не стартует services, не вызывает LLM/Ollama, не требует GPU и не подтверждает API/UI/runtime integration.
...
## Comparison Internal Baseline Matrix

Required local checks:

- normalized document model tests;
- deterministic diff tests;
- Markdown report tests;
- comparison quality gate;
- optional linked parser baseline через `RUN_PARSER_GATE=1`.

Expected status:

- PASS: all comparison tests pass.
- DEGRADED: comparison gate fails but parser/file-chat runtime remains healthy; this blocks any comparison pilot claim.
- FAIL: comparison gate failure plus parser/file-chat regression or uncontrolled errors.

File-chat/live scope:

- comparison engine is not integrated into file-chat/API/UI in this window;
- do not test comparison through browser/API unless future implementation adds it explicitly;
- current validation is internal source/test baseline only;
- no LLM summary over diff, storage, comparison endpoint or comparison UI is validated in this window.
...
- comparison quality gate output;
...
- comparison artifacts from synthetic fixtures only, if saved;
...
Comparison internal baseline PASS:

- normalized document model tests PASS;
- deterministic diff tests PASS;
- Markdown report tests PASS;
- comparison quality gate PASS;
- comparison remains internal only: no API/UI/runtime/file-chat claim.

DEGRADED:

- runtime healthy и chat работает, но file-chat имеет известную OCR quality issue;
- runtime healthy and parser/file-chat remain stable, but comparison gate fails;
- comparison internal baseline is not pilot-claim ready;
...
- comparison quality gate fails because of uncontrolled error;
...
- не разрабатываем comparison API/UI/runtime integration;
- не подключаем comparison engine к file-chat/API/UI без отдельного implementation patch;
...
- не заявляем comparison API/UI/runtime готовыми;
- не заявляем Comparison Engine live-подтверждённым или production-ready.
...
Parser quality gate:
Comparison quality gate:
Comparison internal baseline:
PDF OCR v1 default-off / opt-in:
Metrics summary:
Artifacts path:
Issues:
...
В `Next one action` указать ровно один следующий шаг. После regression plan и validation readiness decision следующий крупный feature-блок выбирается отдельно; comparison API/UI/runtime integration не считать готовой до отдельного implementation и validation approval.
