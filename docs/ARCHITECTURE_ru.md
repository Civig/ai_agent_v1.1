...
- `worker-parser` для parser-stage file processing
- optional `worker-gpu` для GPU-targeted chat execution
- `postgres` для conversation persistence baseline
...
- read-only operator dashboard и dashboard telemetry/history/events endpoints
- health endpoints
...
`postgres` — это текущий persistent conversation storage baseline для runtime, когда включены `PERSISTENT_DB_*` flags. Он:

- хранит `conversation_threads` и `conversation_messages`
- используется для DB-backed thread/message reads при включённом cutover
- участвует в dual-write baseline для conversation updates
- не заменяет Redis control plane полностью и не отменяет Redis fallback semantics
...
Chat history в текущем baseline использует Redis через `AsyncChatStore`, а при включённом persistence profile дополнительно опирается на DB-backed conversation path.

Текущие свойства:

- bounded history retention
- long-term archival backend в репозитории не реализован
- primary model хранения истории уже per-thread: `chat:{username}:{thread_id}`
...
- PostgreSQL runtime, schema и store уже реализованы через `PERSISTENT_DB_*` settings, `persistence/` package и startup wiring в `app`
- fresh install baseline включает `postgres` service и conversation persistence profile с schema bootstrap, dual-write и read-cutover flags
- thread list и message reads уже умеют использовать DB-backed path при включённом cutover; при mismatch/error сохраняется fallback к Redis
- shadow compare hooks и dual-write groundwork уже существуют, поэтому ownership сейчас transitional, а не purely Redis-only
- ownership split по-прежнему зафиксирован отдельно: Redis остаётся owner для queue/control-plane/transient state, а persistent relational DB — target durable owner для dialog/message/meta entities
...
- operator dashboard уже реализован как read-only monitoring surface поверх summary/live/history/events telemetry, но access model и KPI formalization всё ещё остаются отдельными follow-up темами
- session-scoped active-thread pointer, archive и restore как отдельная platform capability пока не реализованы

Целевая server-side модель thread/session для следующего implementation step отдельно зафиксирована в [THREAD_SESSION_MODEL.md](THREAD_SESSION_MODEL.md), storage direction — в [PERSISTENT_STORAGE_DIRECTION.md](PERSISTENT_STORAGE_DIRECTION.md), ownership split — в [STORAGE_OWNERSHIP_SPLIT.md](STORAGE_OWNERSHIP_SPLIT.md), quota direction — в [QUOTA_MODEL_DIRECTION.md](QUOTA_MODEL_DIRECTION.md), queue/concurrency control direction — в [QUEUE_CONCURRENCY_CONTROL_DIRECTION.md](QUEUE_CONCURRENCY_CONTROL_DIRECTION.md), а operator dashboard direction — в [OPERATOR_DASHBOARD_DIRECTION.md](OPERATOR_DASHBOARD_DIRECTION.md).
...
- read-only operator dashboard с live telemetry/history/events
...
- окончательный authoritative ownership cutover conversation data на persistent relational storage без Redis fallback bridge
- durable user/quota/audit entities beyond current thread/message baseline
- отдельный runtime quota layer с per-user/per-group enforcement
- full session-scoped active-thread/archive/restore platform
