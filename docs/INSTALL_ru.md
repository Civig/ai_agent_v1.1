...
Предпочтительный путь установки — `./install.sh`. Для текущей release-candidate линии `v1.2.0` это по-прежнему primary/supported deployment path и reference baseline для релиз-кандидата. `v1.1.0` остаётся более ранним baseline, а финальный tag `v1.2.0` создаётся только после fresh TEST VM validation. Ручное развёртывание возможно, но это вторичный сценарий и он требует большей аккуратности со стороны оператора.
...
- после завершения forced rotation та же local-admin identity может использовать штатный authenticated password-change path `GET/POST /admin/local/change-password`
- этот normal password-change path доступен только для валидной local-admin session без pending rotation; anonymous requests и обычные AD sessions получают deny
- после успешной смены пароля bootstrap secret больше не действует, текущая local-admin session инвалидируется, а дальнейший вход идёт только по новому паролю
...
- первая выбранная модель становится `DEFAULT_MODEL`
- остальные выбранные модели автоматически уходят в secondary pre-pull set

Container startup и container health checks в текущем baseline согласованы: приложение стартует через `start_app.py`, а Compose health checks используют `runtime_healthcheck.py` против live endpoint.

Installer уже пытается выполнить bounded bootstrap для выбранной `DEFAULT_MODEL` и выбранных secondary models. При ручной установке можно запустить:
...
- secondary selected models тоже пытаются pull'иться автоматически после обработки `DEFAULT_MODEL`
...
- installer честно показывает, какие выбранные модели установились, а какие failed и почему
