...
- which operating systems are officially supported and validated on the current baseline
- which operating systems remain supported targets by installer and packaging assumptions, but were not re-validated on every current-baseline version
...
- supported target, but not re-validated on every version of the current baseline
...
- `Officially supported and validated`:
  A concrete OS/version was validated on the current baseline and is an acceptable supported deployment target.
  Конкретная ОС/версия была валидирована на текущем baseline и считается подтверждённым supported target.
- `Supported target, not re-validated on every version`:
  The current installer and packaging assumptions explicitly support this platform family or version range, but the current baseline was not re-validated on every version in that range.
  Текущий installer и packaging assumptions явно поддерживают эту платформу или диапазон версий, но текущий baseline не был повторно валидирован на каждой версии из этого диапазона.
- `Best effort / not validated`:
...
| --- | --- | --- | --- |
| Ubuntu 24.04 LTS | Officially supported and validated | `install.sh` supports Ubuntu `20.04+`; current clean installer validation was completed successfully on Ubuntu 24.04 for revision `eba7ea9` | This is the clearest validated target on the current baseline. |
| Ubuntu 22.04 LTS | Supported target, not re-validated on every version | `install.sh` accepts Ubuntu `20.04+`; docs and installer are Ubuntu-aware | Supported by the current installer path, but not explicitly re-validated on the current baseline. |
| Ubuntu 20.04 LTS and newer Ubuntu LTS within the current installer range | Supported target, not re-validated on every version | `install.sh` requires Ubuntu `20.04+`; docs currently describe Ubuntu `20.04+` as the supported host profile | Do not treat every version in this range as validated just because the installer accepts it. |
| Debian 11 and newer | Supported target, not re-validated on every version | `install.sh` explicitly accepts Debian `11+`; docs and package assumptions include Debian `11+` | Debian is a supported installer target, but the current baseline was not re-validated on every Debian version. |
...
- The current validation baseline already includes a successful clean installer validation on Ubuntu 24.04 for revision `eba7ea9`.
- That validation evidence does not automatically upgrade Ubuntu 22.04, the whole Ubuntu `20.04+` range, or Debian `11+` to `validated`.
...
- For the safest current recommendation, position Ubuntu 24.04 LTS as the primary validated deployment target.
- For implementation planning, Ubuntu `20.04+` and Debian `11+` remain supported installer targets, but avoid presenting every version in those ranges as re-validated on the current baseline.
