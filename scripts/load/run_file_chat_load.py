...
def output_dir_for(args: argparse.Namespace) -> Path:
    if args.output_dir:
        ...
        return path
    return create_artifact_dir(REPO_ROOT / "artifacts" / "load", label=f"file-chat-{args.profile}")
...
def write_bootstrap_failure_artifacts(*, output_dir: Path, profile: dict[str, Any], reason: str) -> int:
    ...
    write_dicts_csv(output_dir / "results.csv", [result], FILE_CHAT_LOAD_RESULT_FIELDS + ["failure_reason"])
...
def run_request(
    *,
    ...
    thread_id = f"load-file-chat-{int(time.time())}-{worker_index:02d}-{request_index:03d}"
    file_path = resolve_repo_path(str(case["file"]))
    started = time.perf_counter()
    try:
        stream = active_client.post_file_chat_sse(
        ...
        result = build_case_result(case=case, kind="file-chat-load", stream=stream, latency_ms=latency_ms, thread_id=thread_id)
    except Exception as exc:
        latency_ms = int(round((time.perf_counter() - started) * 1000))
        result = build_exception_result(case=case, kind="file-chat-load", error=str(exc), latency_ms=latency_ms, thread_id=thread_id)
...
def execute(args: argparse.Namespace) -> int:
    ...
    all_cases = validate_file_chat_cases(args.spec)
    success_cases = [case for case in all_cases if case.get("expected_status") == "success"]
    if args.case_id:
        success_cases = [case for case in all_cases if case["id"] == args.case_id]
    if not success_cases:
        raise RuntimeError("no file-chat success cases selected for load")

    total_requests = int(profile.get("file_chat_cases") or 1)
    concurrency = max(1, min(int(profile["concurrency"]), total_requests))
    ...
    for index in range(total_requests):
        case = success_cases[index % len(success_cases)]
        tasks_by_worker[(index % concurrency) + 1].append((case, index + 1))
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run lightweight live file-chat load profiles.")
    ...
    parser.add_argument("--spec", default=str(REPO_ROOT / "tests/smoke/specs/file_chat_cases.json"))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"FILE_CHAT_LOAD_FAILED: {exc}", file=sys.stderr)
        return 2
