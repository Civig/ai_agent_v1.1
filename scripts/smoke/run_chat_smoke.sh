...
cd "${SMOKE_REPO_ROOT}"
smoke_init_artifact_dir "chat-smoke" >/dev/null
...
"${SMOKE_PYTHON}" "${SMOKE_REPO_ROOT}/scripts/smoke/smoke_runner.py" \
    ...
    --spec "${SMOKE_REPO_ROOT}/tests/smoke/specs/chat_cases.json" \
    --timeout-seconds "${SMOKE_TIMEOUT_SECONDS}" \
    "${tls_args[@]}"
status=$?
set -e

"${SMOKE_REPO_ROOT}/scripts/smoke/collect_metrics.sh" --phase chat || true
